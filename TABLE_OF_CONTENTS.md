...
|---|---|---|
| It's my Tuesday/Thursday-morning routine | `python scripts/run_scrapers.py && python scripts/score_jobs.py && python scripts/send_digest.py` | Auto-scrapes the named-co list **+ runs the Apify LinkedIn discovery channel (3 geo searches)**, filters, scores survivors, **appends rows to the Google Sheet** (Phase C), then **emails the digest of roles ≥6/10** (Phase D). ~10–15 min + ~$1.20 per run (~$1 Anthropic + ~$0.20 Apify; Resend is free). |
