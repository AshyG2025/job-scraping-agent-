...
- **What it is:** Step-by-step guide (~10 min total) for enabling the Phase E cron. Covers the 7 GitHub Actions secrets needed (6 simple values + 1 special case for the Google Sheets service-account JSON which is pasted as the file's *contents*, not a path), then a manual workflow_dispatch trigger to verify end-to-end. Includes a Troubleshooting block mapping each pipeline step's failure mode to the broken secret/setup step. Plus an Optional section on Slack/Discord/SMS notifications (deferred — V1 relies on GitHub's default failure email).
- **How to use it:** Open it once when first enabling Phase E. After that, only reach for it again if you rotate a secret (e.g., Anthropic key rotated locally → also rotate in GitHub) or onboard a contributor who needs their own copy of the workflow.
- **Use cases:**
  - First-time Phase E enablement
  - Secret rotation (any of the 7)
