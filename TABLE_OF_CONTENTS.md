...
>
> **Last updated:** 2026-05-19 (Cost-guardrail session — Phase E first scheduled cron run on 2026-05-19 16:00 UTC scraped 42 net-new roles and hit Anthropic API credit exhaustion mid-scoring, which silently exited 0 and showed the GitHub Actions workflow as green. **Two guardrails shipped:** (a) **Geo filter temporarily restricted to London-only** — US patterns in `scripts/scrapers/common.py` commented out + Seattle/SF Bay LinkedIn search entries in `scripts/run_scrapers.py` commented out + `HARD_FILTERS.md` Section 3 banner added with restore instructions. Cuts typical run cost from ~$3–8 to ~$1–2. (b) **Fail-loud on Anthropic API errors** — `scripts/score_jobs.py` scoring loop now catches `anthropic.APIError` separately, persists partial results, and `sys.exit`s with a helpful diagnostic (credit / key / rate-limit / outage) so the workflow shows red instead of silently green. Other exceptions (JSON parse, etc.) still soft-fail. `QUICK_COMMANDS.md` updated with re-enable-US-geo recipe + new memory `feedback_geo_guardrail.md` + `project_anthropic_credits_incident.md`. No file added/renamed/deleted, so no Section 1/2 changes. Earlier 2026-05-18 (New side initiative scaffolded — `White Label Product/` folder at project root, treated as a **nested independent git repo** (own `.git/`, gitignored from parent). Extension of the V1 Job Scraping Agent exploring whether the framework is replicable as a white-label template/product for other senior PMs. **Phase 0 (Validation) starter shipped:** three docs — (a) `PHASE_0_KILL_CRITERIA.md` (pre-committed GO/PIVOT/KILL bar + per-call scorecard + final decision template; biggest Phase 0 risk is post-hoc bar-softening), (b) `PHASE_0_OUTREACH_DMS.md` (three DM templates — London headhunter referral / Slack-LinkedIn cold / former-colleague warm — plus "what NOT to say" guardrail + pre-call expectation-reset message; all framed as side-of-desk research to avoid implying employment-status change per `feedback_public_artifact_policy`), (c) `PHASE_0_INTERVIEW_GUIDE.md` (30-min behavioural interview structure anchored on Mom Test principles: past behaviour > future intent, revealed WTP > stated WTP, workarounds + referrals = strongest signals; one-page printable cheat-sheet at the bottom). Plus `White Label Product/README.md` as the new project's root doc with the 4-phase roadmap (0: validate → 1: OSS template → 2: paid onboarding → 3: hosted SaaS), file index, and "where to start" routing. Parent `.gitignore` updated to exclude `White Label Product/`. **The V1 agent never reads from this folder** — it's a clean operational separation. Earlier 2026-05-15 (Interview prep infrastructure shipped: (a) New `/interview-deep-dive-prep` slash command at `.claude/commands/interview-deep-dive-prep.md` — HM-grade 4-phase methodology (strategic project selection → role-specific gap inventory → Socratic conversational depth with bank/sharpen/challenge → narrative synthesis to 60s/4min/10min versions). Anchored on the `Product Experinces/` story library. Sister command to `/analyze-jd`: `/analyze-jd` is for *"should I apply?"*, `/interview-deep-dive-prep` is for *"I have an interview, get me ready."* (b) `Product Experinces/` folder upgraded from a `.docx`-drafts-only folder to a structured story library — two new `.md` files: `invoicing-mexico-pc.md` (Mexico PC / e-Invoicing Blueprint deep-dive) + `supplier-experience-wfm-convergence.md` (WFM Vendor Central deep-dive baseline). Plus `Product Experinces/README.md` documents the new structure. Each `.md` file has: quick summary, canonical metrics per Rule 1, competency bridge for various role types, banked beats organized by interview competency, story versions (60s/4min/10min), pre-baked Q&A bank, open items / to-verify, source material references, prep session log. The `.docx` files remain as personal draft narratives. (c) Wolt HM Round 2 interview prep state doc shipped at `London Work folder/Wolt/WOLT_INTERVIEW_PREP_STATE.md` — captures strategic decision (lead with Invoicing, Supplier as cameo), banked content cross-references, to-verify list, open Round 4 questions (10-day window / GIS canonical / 7 validations), how-to-resume note. (d) Two new memory files: `project_wolt_interview_in_flight.md` + `feedback_interview_prep_methodology.md`. Wolt interview target ~2026-05-29. `QUICK_COMMANDS.md` + `MEMORY.md` updated. Earlier 2026-05-14 (Phase E hotfix — `.github/workflows/scrape.yml` updated to write 2 new secrets `PROJECT_BRIEF_MD` + `COMPANY_LIST_MD` to disk before the score step. Root cause: 2026-05-14 first manual trigger crashed in "Score jobs" with `FileNotFoundError: PROJECT_BRIEF.md` — those files are gitignored per CLAUDE.md Rule 7 so they didn't exist on the runner. Fix mirrors the existing `GOOGLE_SHEETS_KEY_JSON` pattern: paste the file's *contents* into a secret, write to disk in the runner. Secret count bumped 7 → 9. New ongoing chore: re-upload the matching secret whenever `PROJECT_BRIEF.md` or `COMPANY_LIST.md` is edited locally (documented in PHASE_E_SETUP.md Step 3b). `docs/PHASE_E_SETUP.md` + `README.md` + `QUICK_COMMANDS.md` updated. Earlier 2026-05-12 — Phase E — GitHub Actions cron SHIPPED. New: `.github/workflows/scrape.yml` runs the full pipeline (scrape → score → email digest) on cron `0 17 * * 2,4` UTC = 9am PT Tue+Thu year-round (drifts to 10am during PDT). Plus `workflow_dispatch:` for manual triggers. State persistence via `actions/cache@v4` for `_local/scraped_seen.json` + `_local/last_sent_digest.json` (Phase B dedup + Phase D re-send guard). `concurrency:` block prevents Sheet write races. Failures email repo owner via GitHub default. Per-run digest.md + scored_results.json uploaded as 14-day artifact. Code-review hardening 3 fixes also shipped same session: null-score crash coercion in send_digest.py + placeholder-URL fallback in send_digest.py + idempotency marker (`_local/last_sent_digest.json` + `--force` flag) in send_digest.py. New: `docs/PHASE_E_SETUP.md` 4-step ~10-min walkthrough (originally 7 GitHub secrets, now 9 post-hotfix above). Same session earlier: Phase D — email digest SHIPPED. New: `scripts/send_digest.py` (with `--dry-run` flag) reads `_local/scored_results.json`, filters to roles ≥6/10 (override via `DIGEST_SCORE_THRESHOLD` in `.env`), renders plain-text body with STRONG/MODERATE sections + per-dim scores + reasoning + apply URL + Sheet link, sends via Resend's REST API. New: `scripts/check_resend.py` diagnostic (mirrors `check_sheets.py`). New: `docs/PHASE_D_SETUP.md` 4-step ~5-min walkthrough. New `.env` vars: `RESEND_API_KEY` (placeholder until user pastes), `DIGEST_RECIPIENT_EMAIL`, `DIGEST_FROM_EMAIL`. No new Python deps — uses existing `requests` library to hit Resend's single POST endpoint. Asset-match snippets intentionally deferred to V2 Resume Agent. Tue/Thu one-liner now chains `&& python scripts/send_digest.py`. Section 2 adds 3 sub-sections under `scripts/` and `docs/`. Same session earlier: Added 3 cos to `COMPANY_LIST.md` per user request: **Ebury** Tier 1 UK/London (Greenhouse `ebury`, ~240 jobs), **Multiverse** Tier 2 UK/Europe (Ashby `multiverse`, ~38 jobs), **American Express** Tier 2 US (no working ATS — same Coinbase/Checkout.com dead-end pattern; relies on Apify LinkedIn discovery for SF Bay + London tech offices). Boku no-op (already wired). Named-co coverage 30 → 32 of 49 (~65%). Greenhouse module count 18 → 19; Ashby 2 → 3. WTTJ work bailed same session (Algolia-only path didn't apply to logged-in WTTJ app — they've moved to a non-Algolia backend); `scripts/check_wttj.py` + `scripts/scrapers/wttj_algolia.py` deleted. Earlier 2026-05-11 (New `/analyze-jd` slash command shipped at `.claude/commands/analyze-jd.md` — recruiter-grade deep JD analysis (6-section structured brief: Experience Level / Domain Alignment / per-skill ✓✓✓ table cross-referenced to canonical flagship metrics / Team Needs with 3-tier gap classification (Manageable / Non-issue / Dealbreaker) / Role Type Fit (catches consulting / sales-enablement / alliance / people-mgmt / marketing role types that score well on the 4-dim rubric but aren't actually IC platform PM work — the Acuity Analytics blindspot) / Recommendation). Distinct from the lightweight `score_jobs.py` matcher: ~$0.30-0.50 per analysis, ~2 min, manually triggered on shortlisted roles only. New `.claude/commands/` Section 2 sub-section added below. QUICK_COMMANDS.md updated. Earlier 2026-05-11 — Eval-feedback ritual wired into QC_PROCESS.md Step 5 — added new sub-bullet 5e with the procedural loop (surface flagged rows → ask "promote as [N]-anchor?" → append to EVAL_SET.md + EVAL_SET_JDS.md → re-run + classify Δ ≤1 vs Δ >1 inversion data points). Sheet columns to watch are now spec'd: `outcome_status` filtered by `outcome_date` >= last QC date OR `add_to_eval_set` = Yes. Suggested anchor mapping locked in by outcome (recruiter_response / phone_screen → 9, interview → 9 + Liberis-style per-dim bumps, offer → 10, rejection-after-strong-signal → score_in_hindsight). Memory `project_eval_feedback_loop` updated to mark this wired in. When Phase E ships, the auto-step should pre-populate the flagged-rows list. Earlier 2026-05-11 — Snorkel.AI ATS recon DONE — Greenhouse slug `snorkelai` confirmed via direct API + careers-page embed. Added to `scripts/run_scrapers.py` Greenhouse Tier 1 block; end-to-end test returned 53 jobs, 3 PM-titled — 1 known eval-set entry (Sr PM Platform) + 2 net-new (Sr PM Data & Quality, Staff PM Marketplace), all SF Bay. Named-co coverage 29 → 30 of 47 (~64%). Greenhouse module count 17 → 18. Phase B leftover #2 closed; only items #3 (Lever/Plaid) and #5 (Workable + PhenomPeople, 4 cos) remain as real coverage gaps. Earlier 2026-05-11 — Phase B leftover cleanup: re-reconned Coinbase Greenhouse 404 + Checkout.com Workday 403 — both confirmed dead-ends via blind probing (Coinbase: all known ATS providers return 404 or empty, Workday tenants return cryptic 422; Checkout.com: same Workday-422 pattern + DNS-dark `careers.checkout.com`). The broken `Coinbase` Greenhouse entry was removed from `scripts/run_scrapers.py` SOURCES (it was 404-ing every run); both entries replaced with multi-line comments documenting the dead-end + LinkedIn Apify fallback. README.md scraper-coverage stat bumped 30/47 → 29/47. `project_phase_b_leftovers` memory updated with 2026-05-11 re-recon results so the next session doesn't repeat the probes. Earlier 2026-05-11 — Completed the v1.2 partial eval re-run: Rippling 3/2 Δ +1 defensible, Snorkel 9/9 Δ 0, Zillow/FUB 7/7 Δ 0, Wise Cards Pay-in 9/9 Δ 0. Combined with the 5 entries that ran 2026-05-09, **v1.2 is now fully validated across 9 of 9** with zero drift > 1 point. Strong signal flagged for next monthly QC: **all 4 callback-anchored 9-targets** (Wise Treasury, Liberis, Snorkel, Wise Cards Pay-in) scored AT 9, not the convention-predicted 8 — the +1 outcome anchor is systematically redundant when JD-vs-profile read is already strong. Secondary note: Rippling target 2 vs model 3 surfaced an EVAL_SET vs SCORING_PROMPT consistency issue (per-dim weighted yields 3 by formula; target encodes a stricter "domain=2 → skip-band" rule than dealbreaker_cap=4 implements). SCORING_PROMPT.md v1.2 iteration log updated. Earlier 2026-05-09 (Added `Ayesha Resume/Cover Letters/` subfolder for per-application cover letters; gitignored alongside the existing `Resume to use repository/`. Section 2 `Ayesha Resume/` sub-section updated to document the new folder. First cover letter saved: Visa Sr PM — Post Purchase Data Integrations & AI. Earlier 2026-05-09: Eval set: added Posting 9 — Wise Sr PM Cards Pay-in Orchestration. Recruiter reached out 2026-05-09 + HM call scheduled week of 2026-05-12 → callback-anchored target **9** (matcher scored 8 in the Session 3.1 LinkedIn first-firing). First eval-set entry surfaced via the discovery channel itself. EVAL_SET.md + EVAL_SET_JDS.md updated; eval-run cost bumped 8 × $0.12 → 9 × $0.12 = ~$1.08. JD body re-fetched live from the Wise SmartRecruiters API since MANUAL_JDS.md had been cleared. SCORING_PROMPT.md v1.1 → v1.2 with the calibration table refreshed to match current EVAL_SET.md state (added Snorkel + Zillow/FUB + Wise Cards Pay-in; Liberis 8 → 9 post-callback). Validated against the 5 of 9 that ran cleanly that day; remaining 4 deferred to 2026-05-11 (above) due to Anthropic API credit exhaustion mid-run. Earlier 2026-05-08 (Apify LinkedIn discovery channel SHIPPED — Phase B leftover #1. New: `scripts/scrapers/linkedin_apify.py` (Ashby pattern: descriptionHtml inlined, no second round-trip), `apify-client>=1.8.0` in `requirements.txt`, `APIFY_API_TOKEN` in `.env`, 3 LinkedIn `/jobs/search` URLs added to `SOURCES` (Seattle / SF Bay / London, all `keywords=Product Manager` + `f_E=4` Mid-Senior + `f_TPR=r2592000` last 30 days), 67 results per source = ~$0.20/run × 2x/wk = ~$1.60/mo. Companies aren't tied to `COMPANY_LIST.md` tiers — Phase C tier lookup falls back to "discovery" automatically. `run_scrapers.py` now `load_dotenv()`s at top so future scrapers needing secrets work the same way. Earlier 2026-05-07 (Phase C — Google Sheets storage SHIPPED. New: `scripts/sheets.py` (writer), `scripts/check_sheets.py` (connection diagnostic), `docs/PHASE_C_SETUP.md` (one-time setup walkthrough). `scripts/score_jobs.py` now appends every scored role to the configured Google Sheet (Decision 3: keep both JSON + Sheet) with the locked 31-column layout (groups A→B→D→C, sub-scores split out, source + tier auto-filled, score_at_application + prompt_version_at_application as Sheet formulas that auto-copy when `applied=Yes`). Sort: scored_date DESC, final_score DESC. `requirements.txt` adds `gspread` + `google-auth`. Earlier 2026-05-07 (small base resume update — commit f7d806e): WFM bullet $9.6M → $17M; FinAuto bullet 3.5M / 80 → 3M / 320 supplier formats. Earlier 2026-05-03 (Session 2.3 — WFM flagship #1 overhaul: PROJECT_BRIEF.md + PROJECT_BRIEF_PUBLIC.md flagship #1 (WFM Vendor Central Integration Platform) rewritten against `Product Experinces/Supplier deep dive_Prep_v3.docx`. Architecture corrected ("3-layer" → "monolith-first with three planned service extractions"); API claim corrected ("3 named APIs from day one" → "pilot was portal-only, programmatic APIs progressive in Wave 2"); scaling curve added (5 → 150 → 450 → 900 → 1,000+ over 18 months); $600K pilot outcome / team size / churn-NPS metrics added; $20M/$17M canonical reconfirmed by user. **10 new problem-shape framings (#9–#18)** added to flagship #1 (8 → 18) — unlocks distributed-systems, multi-tenant SaaS, ledger-migration, config-platform JD recognition. Stale "Three flagship" comment fixed. Resume non-update settled (user corrects $9.6M → $17M per-application). New memory `project_wfm_canonical_reconciliation.md` captures the resolution. Earlier 2026-05-03 (Session 2.2.2 + post-rerun cleanup): (a) added Posting 7 Snorkel.AI Sr PM Platform (Bay Area, H1B confirmed direct from HM + recruiter, target 9 — HM-callback after warm referral); (b) added Posting 8 Zillow/FUB Sr PM AI Platform & Ecosystem (Seattle, H1B sponsor, initially targeted 8 then retuned 8 → 7); (c) upgraded Posting 3 Liberis 8 → 9 after recruiter callback. Documented callback-anchor Δ -1 convention in EVAL_SET.md "How to use". New Tier 1 sub-section "AI / Data infrastructure growth-stage — US (Series C+)" added to COMPANY_LIST.md (Snorkel) + mirrored to PUBLIC. Women in Tech Jobs added to Always-on aggregators. Eval set 8 entries: 6 perfect Δ 0, 1 expected callback-anchor Δ -1 (Snorkel), 1 retuned to Δ 0 (Zillow). Periodic Sheets → Eval-set feedback loop logged to memory for Phase C activation. Earlier 2026-05-02 (Session 2.2.1): extracted 6 eval-set JD bodies from `Job Posting samples/JD mapping to my exp..docx` into `docs/EVAL_SET_JDS.md` — calibration is now runnable end-to-end via `cat docs/EVAL_SET_JDS.md >> MANUAL_JDS.md && python scripts/score_jobs.py`, no longer dependent on original postings still being live. Earlier 2026-05-02 (Session 2.2): added "Problem-shape framings" sub-block to each of the 4 flagships in `PROJECT_BRIEF.md` + `_PUBLIC.md` (30 framings total); `SCORING_PROMPT.md` v1.0 → v1.1 with new "Situational matching — read the problem-shape, not just the keywords" section + 4 explicit guardrails. Calibration: Liberis 9, Wise Treasury 9, PayPal 8, Salesforce MuleSoft 7 (-1 from v1.0; guardrail working). Earlier Session 2.1.4: Ashby module shipped at `scripts/scrapers/ashby.py` covering Ramp + Deliveroo (Tier 1). SOURCES now 30 named-co scrapers (28 → 30). Ashby's listing endpoint inlines `descriptionHtml`, so `fetch_jd_body()` returns a cached value — no second HTTP round-trip per job. First live run: 1 net-new JD (Deliveroo Staff PM, Ads, London). Earlier 2026-05-01: Session 2.1.3: Workday module at `scripts/scrapers/workday.py` covering 8 tenants — Visa, PayPal, Salesforce, Zillow, Target, Capital One, Walmart, Zoom. Module contract refactored: `fetch_listing` / `fetch_jd_body` take the full source dict instead of `(slug, company_name)`. SOURCES bulk-expanded to 28 named-co scrapers — Tier 1 + Tier 2 ATS recon + Boku added to COMPANY_LIST.md Tier 1 UK/London. Welcome to the Jungle added to discovery aggregators. Earlier 2026-04-30: Phase B shipped: `scripts/run_scrapers.py` orchestrator + `scripts/scrapers/` package with `common.py`, `greenhouse.py`, `smartrecruiters.py`; QC_PROCESS gains a 30-sec scraper-volume sanity check; requirements.txt adds `requests`. Same day: portfolio prep — gitignored `MANUAL_JDS_PROCESSED.md`, `Ayesha Resume/Resume to use repository/`, `COMPANY_LIST.md`, `PROJECT_BRIEF.md` as local-only working copies; added committed `*_PUBLIC.md` framework copies; added CLAUDE.md Rule 7; refreshed README Status + added platform-PM-lens framing subsection)
...
|---|---|---|
| It's Tuesday/Thursday morning | (nothing — the cron runs automatically at 9am PT) | Phase E cron at `.github/workflows/scrape.yml` runs the full pipeline. The digest lands in your inbox + new rows hit the Sheet. If anything fails, GitHub emails you. |
| I want to manually trigger the cron (don't wait for Tue/Thu) | https://github.com/AshyG2025/job-scraping-agent- → Actions tab → "Job scraper — Tue/Thu morning" → "Run workflow" | Useful after editing `COMPANY_LIST.md`, `SCORING_PROMPT.md`, etc. — verify in CI before the next scheduled run |
| I want to run the full pipeline locally instead of via cron | `python scripts/run_scrapers.py && python scripts/score_jobs.py && python scripts/send_digest.py` | Same chain as the cron. Useful for debugging or one-off runs while offline. ~10–15 min + ~$1.20 per run. |
...
| I receive the digest email | Email inbox + Google Sheet | Read top matches; mark `applied` Yes/No in the Sheet (~5 min). Sent via `python scripts/send_digest.py` (Phase D). |
| I want to preview the digest body without emailing | `python scripts/send_digest.py --dry-run` | Prints subject + body to terminal off the most recent `_local/scored_results.json`. Useful before sending or for debugging the formatter (~2 sec). |
| The digest email never arrived | `python scripts/check_resend.py` | One-line connection diagnostic; sends a short test email and surfaces the broken Phase D setup step (~5 sec) |
| The cron stopped running (no Tuesday email) | https://github.com/AshyG2025/job-scraping-agent-/actions | Open the Actions tab, click the most recent failed run, see which step broke. Troubleshooting matrix is in `docs/PHASE_E_SETUP.md`. |
| I want to download a past run's output for post-mortem | Actions tab → click a run → Artifacts (bottom of page) → `pipeline-output-<run-id>` | Contains `_local/digest.md` + `_local/scored_results.json`. 14-day retention. |
...
| When this happens... | Go to... | What you'll do |
|---|---|---|
| I have an HM / panel interview scheduled and want to be ready | `/interview-deep-dive-prep` (slash command) | Runs the 4-phase methodology (strategic project selection → role-specific gap inventory → Socratic conversational depth → narrative synthesis). Anchors on `Product Experinces/` story library |
| I want to see what's already banked for a project across past interviews | `Product Experinces/<project>.md` | Each file has banked beats, story versions, Q&A bank, prep session log |
| I'm resuming an interview prep paused mid-session | `London Work folder/<Company>/<COMPANY>_INTERVIEW_PREP_STATE.md` | The state doc has banked content references + open Round N questions + how-to-resume note |
| I want to add a new flagship project to the library | Create `Product Experinces/<slug>.md` following existing structure + update `Product Experinces/README.md` table + add a sub-section in Section 2 of this TOC | See `Product Experinces/README.md` for the convention |
...
#### `scripts/send_digest.py` *(Phase D — email digest sender)*
- **What it is:** Reads `_local/scored_results.json` (Phase A's output), filters to roles ≥`DIGEST_SCORE_THRESHOLD` (default 6), sorts by score DESC, renders a plain-text email body with two sections — STRONG (≥8) and MODERATE (6–7) — and sends it to `DIGEST_RECIPIENT_EMAIL` via Resend's REST API. Each role shows score / verdict / per-dim breakdown / 1-line reason / H1B note / apply URL; top of email links to the Google Sheet (pulled from `GOOGLE_SHEET_ID`). If 0 roles meet the threshold, no email is sent (logs a console note instead). Standalone — not auto-fired from `score_jobs.py` so you can re-send without re-scoring. Uses the existing `requests` library (no new Python deps).
- **How to use it:** `python scripts/send_digest.py` to send. `python scripts/send_digest.py --dry-run` to print body to terminal without sending. Runs after `score_jobs.py` (or any time after a scoring run).
- **Use cases:**
  - Tue/Thu morning routine: chain after `score_jobs.py` to email yourself the day's matches.
  - Re-sending the same digest if the first send hit Resend's rate limit or your inbox lost it.
  - Testing a formatting tweak: edit `render_role()` / `render_email_body()` then `--dry-run` to preview.
- **When to edit:**
  - Changing the score threshold default → edit `DEFAULT_THRESHOLD` (better: just set `DIGEST_SCORE_THRESHOLD` in `.env`).
  - Changing the email format → edit `render_role()` for per-role layout, `render_email_body()` for section structure, `render_subject()` for subject line.
  - Switching to HTML email → add an `html` field alongside `text` in `send_via_resend()`'s JSON payload.

#### `scripts/check_resend.py` *(Phase D — connection diagnostic)*
- **What it is:** Tiny one-off script. Loads `.env`, sends one short test email via Resend's REST API, prints `✅ Sent successfully — Resend ID: …` on success or a clear `❌` message pointing at the broken setup step on failure.
- **How to use it:** `python scripts/check_resend.py`. Run it after `docs/PHASE_D_SETUP.md` Step 4, or any time `send_digest.py` stops delivering emails.
- **When to edit:** Almost never. If the diagnostic surfaces a new failure mode worth catching, add a labeled error case before the `requests.post()`.

### `.github/workflows/` folder (GitHub Actions cron — Phase E)

#### `.github/workflows/scrape.yml`
- **What it is:** The Phase E cron workflow. Runs the full pipeline (`run_scrapers.py && score_jobs.py && send_digest.py`) on a schedule of `0 17 * * 2,4` UTC = 9am PT Tue + Thu year-round (drifts to 10am during PDT). Also exposes `workflow_dispatch:` for manual triggers via the Actions tab. State persistence via `actions/cache@v4` for `_local/scraped_seen.json` (Phase B dedup state) + `_local/last_sent_digest.json` (Phase D re-send guard) — without this, every Tuesday's run would re-surface roles surfaced last Thursday. `concurrency:` block prevents overlapping runs from racing on the shared Google Sheet. Per-run output (digest.md + scored_results.json) is uploaded as a `pipeline-output-<run-id>` artifact with 14-day retention for post-mortems. Failures email the repo owner via GitHub's default notification.
- **How to use it:** You don't run it manually — the cron runs itself. To trigger ad-hoc: GitHub repo → Actions tab → "Job scraper — Tue/Thu morning" → "Run workflow". To watch a run live: click into the run from the Actions list.
- **Use cases:**
  - Hands-off Tue/Thu morning routine (the whole pipeline, no terminal)
  - Manual trigger after editing `COMPANY_LIST.md` / `SCORING_PROMPT.md` to verify changes work in CI before relying on the next scheduled run
  - Debugging from CI logs when a step fails (richer info than local error output)
- **When to edit:**
  - Changing the cron schedule → edit the `cron:` line; remember it's UTC
  - Adding a new secret env var (e.g., a 6th tier of API key) → add a `secrets.NAME` reference to the relevant step's `env:` block + add the secret in GitHub UI
  - Changing the pipeline order or adding a step → edit the relevant `- name:` block; mind the order (cache restore must come before any script run; cache save happens automatically at end of job)

### `.claude/commands/` folder (Claude Code slash commands, project-scoped)

#### `.claude/commands/analyze-jd.md`
- **What it is:** A custom slash command — `/analyze-jd <paste JD text or URL>` — that produces a recruiter-grade deep analysis of a single JD against your profile. Distinct from the lightweight `scripts/score_jobs.py` matcher: deeper, more structured, more expensive (~$0.30-0.50 per analysis vs ~$0.12 for the lightweight score), and manually triggered on shortlisted roles only (typically score ≥ 7 from the lightweight matcher, or any role you're considering applying to).
- **What it produces:** A 6-section structured brief — (1) Experience Level Match, (2) Platform Domain Alignment with STRONG/MEDIUM/GAP labels, (3) Per-skill ✓✓✓ table cross-referenced to specific canonical flagship metrics (the Liberis-style table from `Job Posting samples/JD mapping to my exp..docx`), (4) Team Needs & Critical Gaps with 3-tier classification (Manageable / Non-issue / Dealbreaker), (5) Role Type Fit (catches consulting / sales-enablement / alliance / people-mgmt / marketing role types that score well on the 4-dim rubric but aren't actually IC platform PM work — the Acuity-Analytics blindspot), (6) Recommendation + Resume Choice + Cover Letter Angle.
- **How to use it:** In any Claude Code session in this folder, type `/analyze-jd` followed by the JD body or URL. Claude loads `PROJECT_BRIEF.md` + `COMPANY_LIST.md` + `SCORING_PROMPT.md` automatically as context. The output renders in the conversation; save what's useful (e.g., promote into `_drafts/` or use as the input to a cover letter draft).
- **Use cases:**
  - You scored a 7 or 8 on the lightweight matcher and want to know whether to actually apply
  - The matcher gave a confusing or borderline result and you want a second opinion before time-investing
  - You're preparing for a recruiter call and want structured talking points
  - You want to feed the analysis into the V2 Resume & Cover Letter Expert Agent (when shipped) as the input brief
- **When to edit:** When the analysis framework needs sharpening (e.g., adding a new section like Trajectory Fit, or changing the Role Type Fit categories). Bump no version — slash commands aren't versioned.

#### `.claude/commands/interview-deep-dive-prep.md`
- **What it is:** A custom slash command — `/interview-deep-dive-prep` — that runs HM-grade interview prep for a specific role. Sister command to `/analyze-jd`: `/analyze-jd` answers *"should I apply to this role?"*; `/interview-deep-dive-prep` answers *"I have an interview scheduled, get me ready."* Anchored on the `Product Experinces/` story library — each interview enriches the relevant project file with new banked beats.
- **What it produces:** A structured 4-phase prep session: (1) **Strategic project selection** — scores eligible projects across role-specific competency match, platform/systems thinking, ambiguity, cross-functional scope, outcomes, story arc, quick-on-feet under probe; produces a decisive recommendation on which project leads + cameo strategy. (2) **Role-specific gap inventory** — for the chosen project, identifies what THIS role's HM will probe vs. what's already banked; marks 5-7 as 🔴 critical and the rest as 🟡 useful. (3) **Conversational depth** — Socratic rounds of 2-3 questions, with bank ✅ / sharpen 🟡 / challenge rhythm; pushes back twice on the same point when HM-grade probes warrant it. (4) **Narrative synthesis** — assembles chronologically into 60s / 4min / 10min versions, builds the bridge inventory for JD dimensions not directly covered, pre-empts the 5-7 most likely HM probes. (5) **Write back** — updates the project `.md` file in `Product Experinces/` with new banked beats + creates a role-specific state doc at `London Work folder/<Company>/<COMPANY>_INTERVIEW_PREP_STATE.md`.
- **How to use it:** In any Claude Code session in this folder, type `/interview-deep-dive-prep`. Claude loads `PROJECT_BRIEF.md` + `COMPANY_LIST.md` + `CLAUDE.md` automatically + lists `Product Experinces/*.md` to see what's banked. Then asks for: target role, interview format, candidate projects to consider. Methodology stays conversational — Ayesha has explicitly opted in to HM-grade pushback (see memory `feedback_interview_prep_methodology.md`).
- **Use cases:**
  - You have an HM round / panel scheduled for a Tier 1 role and want to be ready
  - You scored well on `/analyze-jd` and have a recruiter call — want a 60s opener anchored on the right project
  - You're prepping a deep-dive STAR for an upcoming take-home or case round
  - You want to enrich a project's banked beats over multiple interviews (each session adds to `Product Experinces/<project>.md`)
- **When to edit:** When the methodology needs sharpening (e.g., changing the question-batch rhythm, adding a new gap-category bucket, changing the write-back convention). The skill is registered at `~/.claude/skills/` automatically; project-level changes happen here.
...
#### `docs/PHASE_E_SETUP.md` *(Phase E — one-time GitHub secrets walkthrough)*
- **What it is:** Step-by-step guide (~10 min total) for enabling the Phase E cron. Covers the 9 GitHub Actions secrets needed (6 simple values + 1 special case for the Google Sheets service-account JSON pasted as the file's *contents* + 2 markdown-content secrets `PROJECT_BRIEF_MD` and `COMPANY_LIST_MD` that mirror the gitignored `PROJECT_BRIEF.md` + `COMPANY_LIST.md` files — added 2026-05-14 after the first manual cron crashed with `FileNotFoundError`), then a manual workflow_dispatch trigger to verify end-to-end. Includes a Troubleshooting block mapping each pipeline step's failure mode to the broken secret/setup step. Plus an Optional section on Slack/Discord/SMS notifications (deferred — V1 relies on GitHub's default failure email).
- **How to use it:** Open it once when first enabling Phase E. After that, reach for it again when (a) rotating a secret (e.g., Anthropic key rotated locally → also rotate in GitHub), (b) editing `PROJECT_BRIEF.md` or `COMPANY_LIST.md` locally — you must also update the matching markdown-content secret or the cron uses stale content, or (c) onboarding a contributor who needs their own copy of the workflow.
- **Use cases:**
  - First-time Phase E enablement
  - Secret rotation (any of the 9)
  - **Re-uploading `PROJECT_BRIEF_MD` / `COMPANY_LIST_MD` after editing the local files** (recurring chore)
  - Re-doing setup on a fresh repo or fork
- **When to edit:** When the secret list changes (e.g., a new env var is added to one of the scripts), or when the workflow file's secret-injection pattern changes.

#### `docs/PHASE_D_SETUP.md` *(Phase D — one-time setup walkthrough)*
- **What it is:** Short ~5-min, 4-step guide for wiring up Resend so `python scripts/send_digest.py` can email you the digest. Covers Resend signup → API key creation → 3 env-var additions to `.env` → test-send via `python scripts/check_resend.py`. Also includes an Optional section on verifying your own domain later (only worth doing if deliverability becomes a problem; the default `onboarding@resend.dev` sender is fine for personal-volume usage). Includes a Troubleshooting block mapping common errors (401, 422, missing env var) to the step that fixes them.
- **How to use it:** Open it once when first setting up Phase D. After that, only reach for it again if you need to rotate the Resend API key, change recipient address, or re-do setup on a new machine.
- **Use cases:**
  - First-time Phase D setup
  - Resend API key rotation (revoke + recreate via Resend dashboard, paste new key in `.env`)
  - Switching the recipient email address
  - Onboarding the project to a new machine
- **When to edit:** When the Phase D setup process changes (new env var, switching email provider, custom domain verified).
...
#### `Product Experinces/`
- **What it is:** Source-of-truth artifacts for your flagship projects + a structured `.md` story library used by `/interview-deep-dive-prep`. As of 2026-05-15, the folder has **two layers**: (a) the existing `.docx` and `.pdf` files (PRDs, reference guides, Ayesha's personal narrative drafts) and (b) new structured `.md` files (one per project, designed for reuse across interview cycles).
- **Structured `.md` files (the story library):**
  - `Product Experinces/README.md` — folder index + naming conventions + how to add a new project
  - `Product Experinces/invoicing-mexico-pc.md` — Mexico Payment Complement / e-Invoicing Blueprint deep-dive (banked through Wolt HM Round 2 prep Round 3)
  - `Product Experinces/supplier-experience-wfm-convergence.md` — WFM Vendor Central Integration Platform deep-dive baseline (depth still lives in `London Work folder/Wolt/Project context/Supplier exp./Cards_Orchestration_HM_Prep_v1.7.docx` until next prep session that leads with this project)
- **Each `.md` file has:** quick summary, canonical metrics (per Rule 1), competency bridge for various role types, banked beats organized by interview competency, story versions (60s / 4min / 10min), pre-baked Q&A bank, open items / to-verify, source material references, prep session log.
- **How to use it:** Read-only when verifying a metric or claim. **Never let Claude invent numbers — always verify against these files + `PROJECT_BRIEF.md`.** For interview prep, run `/interview-deep-dive-prep`. For resume / cover-letter work, defer to the `.md` files and `PROJECT_BRIEF.md` over the older `.docx` drafts (which may have stale / unreconciled numbers).
- **Use cases:**
  - You wrote a bullet claiming "$9.6M impact" — verify the exact figure against `PROJECT_BRIEF.md` § Flagship #1 (canonical is $20M / $17M per memory `[[project-wfm-canonical-reconciliation]]`)
  - You want to recall the original PRD for Mexico Tax Recon → `FinAuto/` or `Mexico Reconciliation Invoice story.docx`
  - You have an interview scheduled → run `/interview-deep-dive-prep` which loads the right `.md` files
- **When to edit:** When you're adding a new flagship project (create a new `.md` + update README + add a TOC sub-section); when an interview prep session enriches a project file (the `/interview-deep-dive-prep` command handles writes automatically); when canonical metrics change (cross-reference `PROJECT_BRIEF.md`).
...
### Side initiatives (out of scope for the V1 agent)

#### `White Label Product/` *(nested git repo)*
- **What it is:** Extension of the V1 Job Scraping Agent exploring whether the framework is replicable as a white-label template / paid product for other senior PMs. **Operationally independent** — has its own `.git/` inside the folder, gitignored from the parent so the two repos don't share commit history. If validated, the nested repo can be pushed to its own GitHub remote with clean history.
- **Current status:** **Phase 0 — Validation.** 10 customer-discovery interviews to produce a GO / PIVOT / KILL decision. No code being written; pre-build of any product is gated on Phase 0 GO. Subsequent phases (1: OSS template, 2: paid onboarding, 3: hosted SaaS) live in the roadmap but are only built if the previous phase validates.
- **How to use it:** Open `White Label Product/README.md` first — it's the project-root doc for the nested repo and indexes the Phase 0 starter files. **The V1 agent never reads from this folder.** When working on V1 (scoring, scrapers, eval, interview prep), ignore it entirely.
- **When to edit:** Only when working on the white-label initiative itself. Commits in this folder go to the nested repo, not the parent.
