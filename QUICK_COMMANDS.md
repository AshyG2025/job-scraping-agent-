...
>
> **Last updated:** 2026-05-19 (Cost guardrail session — added "Re-enable full Seattle / SF / London geo scope" recipe to Git & GitHub section, after geo filter was temporarily restricted to London-only on 2026-05-19 as a cost guardrail. Also added "Diagnose a green-but-empty workflow run" entry pointing to the typical root causes — Anthropic credit exhaustion, stale dedup cache, or all roles below threshold.) Earlier 2026-05-15 (Added `/interview-deep-dive-prep` slash command — HM-grade interview prep methodology anchored on `Product Experinces/` story library. Distinct from `/analyze-jd` (which is for *"should I apply?"*); this one is for *"I have an interview, get me ready."* Plus `Product Experinces/` is now a structured `.md` library (one file per banked project) alongside the existing `.docx` drafts. Earlier 2026-05-14 (Phase E hotfix — added "Re-upload `PROJECT_BRIEF_MD` / `COMPANY_LIST_MD` after editing local file" entry to Git & GitHub section. The Phase E cron crashed on its first manual trigger because `PROJECT_BRIEF.md` and `COMPANY_LIST.md` are gitignored per Rule 7 — fix is 2 new secrets that hold the file *contents*, re-uploaded whenever the local file changes. Earlier 2026-05-12 — Phase E — GitHub Actions cron shipped. New entries in "Git & GitHub" section: manual cron trigger via Actions tab, viewing past runs, downloading artifacts, disabling the cron. Earlier 2026-05-12 — Phase D shipped: new `python scripts/send_digest.py` (with `--dry-run`) + `python scripts/check_resend.py` diagnostic. Tue/Thu one-liner now chains `&& python scripts/send_digest.py`. Earlier 2026-05-11 — Added `/analyze-jd` slash command — recruiter-grade deep JD analysis, distinct from the lightweight `score_jobs.py` matcher.)
...
| **HM-grade interview prep for an upcoming round** | `/interview-deep-dive-prep` — runs the 4-phase methodology: strategic project selection from `Product Experinces/` → role-specific gap inventory → Socratic conversational depth (bank/sharpen/challenge) → narrative synthesis (60s/4min/10min). Updates the project file with new banked beats; creates a role-specific state doc in the company folder. Lives in `.claude/commands/interview-deep-dive-prep.md`. Use when you have a real interview scheduled. |
| Resume an interview prep paused mid-session | *"Resume {Company} interview prep — see `London Work folder/{Company}/{COMPANY}_INTERVIEW_PREP_STATE.md` and the project file it references in `Product Experinces/`. Pick up at Round N."* | The state doc has the open Round N questions and what's banked so far |
| See banked content for a project across all past interviews | Open `Product Experinces/<project-slug>.md` | Each file has banked beats, story versions, Q&A bank, source references, and a prep session log. Library grows over time as new interviews enrich it. |
...
| **Re-enable full Seattle / SF / London geo scope** (currently London-only since 2026-05-19) | 1) Uncomment US patterns in `scripts/scrapers/common.py:60-77` 2) Uncomment Seattle + SF Bay entries in `scripts/run_scrapers.py:~95-106` 3) Revert the banner + restore the KEEP list in `HARD_FILTERS.md` Section 3 4) Bump `HARD_FILTERS.md` "Last updated" 5) Clear scraped_seen.json cache before the next run if you want to re-surface US roles posted during the London-only period | Restricted as a cost guardrail after the 2026-05-19 run hit Anthropic credit exhaustion on 42 net-new roles. Restore when you're confident in steady-state weekly cost. |
| **Diagnose a green-but-empty workflow run** (no email, no Sheet rows) | Open the failed run → click "Score jobs" step → search logs for `Anthropic` errors or `_error`. Also open the run's `pipeline-output-*` artifact → check `digest.md` for per-role error messages | Most common causes: (a) Anthropic API credit exhausted → top up at https://console.anthropic.com/settings/billing (b) dedup cache has all today's URLs marked seen → 0 net-new → empty digest (c) all roles scored < threshold → email correctly suppressed. After 2026-05-19, Anthropic API errors fail the workflow loud (red ❌). |
