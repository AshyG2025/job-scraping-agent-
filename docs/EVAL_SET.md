...
>
> **Last updated:** 2026-05-09 (Session 3.2 — added Posting 9 Wise: Sr PM Cards Pay-in Orchestration. Recruiter reached out 2026-05-09 + HM call scheduled week of 2026-05-12 → callback-anchored target **9** (matcher scored 8 in the Session 3.1 first-firing of the LinkedIn discovery channel; +1 outcome anchor lifts target). Uses the **Snorkel pattern** (per-dim reflects JD-vs-profile fit, +1 only on `final_score`) — this is a NEW callback-anchored entry, NOT an upgrade of an existing one (the upgrade pattern with per-dim bumps is documented in Posting 3 Liberis). Eval set now: **9 entries**, 4 callback-validated 9-anchors (Wise Treasury, Liberis, Snorkel, Wise Cards Pay-in). Earlier 2026-05-03 (Session 2.2.2 + post-rerun cleanup: (a) added Posting 7 Snorkel.AI Sr PM Platform (HM-callback after warm referral, target 9; H1B `confirmed_sponsor` via direct HM + recruiter contact); (b) added Posting 8 Zillow/FUB Sr PM AI Platform & Ecosystem (user-judged good fit) — initially targeted 8 then **retuned 8 → 7** after eval re-run scored 7 with defensible LLM-agent-memory-gap reasoning, walking back the overconfident "Session 2.0 doubly anchored" claim; (c) **upgraded Posting 3 Liberis 8 → 9** after recruiter callback, per-dim Skills + Team needs each 8→9 per Wise Treasury precedent. (d) Documented the **callback-anchor Δ -1 convention** in §"How to use" — Snorkel target 9 / model 8 is now legible as design-by-convention, not drift. Eval set: 8 entries, 3 callback-validated 9-anchors (Wise Treasury, Liberis, Snorkel), all 6 non-callback entries within Δ 0.)

---

## Why these 9 postings

This set was deliberately chosen to span the full scoring range — from "recruiter / hiring manager actually reached out" (a 9) through "weak match, manager-scope dealbreaker" (a 2). The set now contains **four callback-confirmed 9-anchors** (Wise Treasury via recruiter, Liberis via recruiter, Snorkel via HM after warm referral, Wise Cards Pay-in via recruiter + HM call), giving the matcher multiple data points on the upper band — increasingly important now that the eval set is doubling as a ground-truth feedback loop, not just a static fixture. It also includes a tricky case (Boku) where surface-level signals look moderate but a hard-requirement dealbreaker (Mandarin fluency) drops the score. Posting 7 (Snorkel) adds the first US-based entry + the first instance of *hiring-manager* outreach + the only ML-data-platform domain in the set, so it stretches the rubric on AI/ML adjacency. Posting 8 (Zillow/FUB) adds the first LLM-agent-platform US/Seattle entry; after the 2026-05-03 eval re-run it was retuned 8 → 7 to match the model's defensible reasoning that the JD's LLM-agent-memory productization framing is genuinely a stretch beyond Ayesha's classical-ML AI Invoice flagship. It now sits as a calibration twin to Posting 4 (Ebury, target 6) — both adjacent platform domains; Zillow is one notch up because the platform-architecture half is direct + H1B and Referral Network coverage. Posting 9 (Wise Cards Pay-in Orchestration) is the first eval-set entry surfaced **via the LinkedIn discovery channel itself** (Session 3.1, 2026-05-08 — i.e., the channel produced its own ground-truth example) and adds a card-payments-orchestration domain — adjacent FinTech rails with a card-acquirer-specific knowledge gap that the matcher correctly read as an honest 8 before the recruiter callback validated the platform-fit half. The matcher needs to catch all of these.
...
## Posting 9 — Wise: Senior Product Manager — Cards Pay-in Orchestration

**User label:** Recruiter reached out + HM call scheduled week of 2026-05-12 (callback validation, 2026-05-09)
**Target final score:** **9** (`prioritize`)
**Callback context (added 2026-05-09):** A Wise recruiter reached out to Ayesha for this role and the HM call is scheduled for the week of 2026-05-12. Per the eval-set callback-anchor convention, the +1 outcome anchor lifts target from the matcher's JD-vs-profile read of 8 to **9**. This entry uses the **Snorkel pattern** (per-dim reflects the JD-vs-profile fit at first-pass; the +1 lives in `final_score` only) — no per-dim adjustments — because this is a NEW eval-set entry, not an upgrade of an existing one (the upgrade pattern with per-dim bumps is documented in Posting 3 Liberis). Δ -1 vs. matcher is the expected design behavior, not drift.
**Provenance:** First eval-set entry surfaced via the LinkedIn discovery channel itself (Session 3.1 first-firing, 2026-05-08).

| Dimension | Target | Reasoning |
|---|---|---|
| Domain | 7 | Multi-acquirer card-payments orchestration is a FinTech-rails platform — multi-system integration shape is a direct match to her flagships. Adjacent step: card acquiring specifically (issuer / scheme / regional economics, EMV/SCA, acquirer-portfolio cost models) is a real depth gap. Closest analog she has is FinAuto E-Invoicing's 7 RS patterns balancing clearance models + government-API availability across 15+ countries — that's a portfolio-of-architectures problem in spirit, not card acquiring in particular. Honest 7 — not a 6 (the routing-platform shape is direct), not an 8 (no card-acquirer evidence). |
| Skills | 8 | JD asks for: define multi-year orchestration vision, move from static rules to dynamic ML routing, hands-on with engineers/analysts, balance regulatory + compliance + engineering trade-offs. Direct fits: rules→ML evolution at scale (AI Invoice Automation: manual baseline → 80%+ ML accuracy across 3M invoices/year), multi-system routing under constraints (WFM Convergence 5→1, Mexico Tax Recon 36-attribute mapping under regulated portals). Senior IC technical-fluency requirement matches her bread and butter. |
| Level | 8 | "Senior PM" + JD-tagged "Mid-Senior Level"; 7 yrs PM at-bar; IC-only (her hard filter); London no-visa. Clean Senior IC bullseye. |
| Team needs | 8 | Team needs leading the rules→ML routing transition + managing acquirer portfolio across new markets / schemes / product types + balancing acquirer cost-performance. Her flagship evidence covers two of three directly: the rules→ML transition pattern (AI Invoice) and the portfolio-of-trade-offs pattern (FinAuto E-Invoicing 7 RS patterns balancing clearance models / API availability / engineering cost). Gap: card-acquirer-specific portfolio depth — recruiter callback partially validates that the team sees her platform / integration background as a needs-fit anyway. |

**Expected verdict:** `prioritize` (the JD-vs-profile read is `apply` at 8; the +1 outcome anchor lifts to `prioritize` at 9)
**Resume choice:** `Resume to use repository/Ayesha CV Wise Invoicing role.pdf` (per matcher's asset-match recommendation; pre-tweaked Wise variant lives in repo from prior application)
**H1B status:** `n/a_uk_role`
**Lead with:** Mexico Tax Reconciliation (multi-system routing/reconciliation under regulatory constraints — closest structural analog to "routing payments between acquirers, adjusting for regional / issuer-level differences") + AI Invoice Automation (rules→ML evolution at scale — closest structural analog to "moving from static rules to dynamic, machine-learning routing"); supporting = FinAuto E-Invoicing Blueprint (7 RS patterns balancing clearance models + government-API availability + engineering cost — analogous to managing an acquirer portfolio across regions/schemes).
**Notable cover-letter angle:** "Building a routing platform that evolves from static rules to dynamic ML without disrupting live volume is the same sequencing problem I owned on AI Invoice Automation (manual baseline → 80%+ ML accuracy across 3M invoices) and the WFM Convergence platform (5 legacy systems → 1 unified surface, 1,000+ partners, no disruption). I haven't worked on card acquiring specifically — but multi-system financial routing under regulatory constraints is the through-line of my last 4 years."
**Notable gap to address:** Card-acquiring depth (acquirer economics, scheme/issuer dynamics, EMV/SCA). Cover letter should explicitly name the gap and bridge via the 7-RS-patterns framing — show comfort with portfolio-level architecture trade-offs across regional / regulatory variation, even if the variation she's worked with was government tax portals not card schemes.

---

## How to use this file when iterating

1. After any meaningful change to `SCORING_PROMPT.md`, run the prompt against the 9 postings:
